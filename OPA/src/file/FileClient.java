 ...
 */
public class FileClient implements RemoteFileClient{
	...
	FileClient.fromClientSentFilename = fileName;
    }
    
    public void startClient(String hostName, int portNumber) throws Exception {
	FileClient client = new FileClient();
	RemoteFileClient stubClient = (RemoteFileClient) UnicastRemoteObject.exportObject(client, 0);
	
	// bind to registry
	registry = LocateRegistry.getRegistry(hostName, portNumber);
	registry.bind("RemoteFileClient", stubClient);
    }
    @Override
